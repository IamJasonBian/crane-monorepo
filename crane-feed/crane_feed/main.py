...
def main():
    ...
    # Start Best Buy product monitor in background (30 min intervals)
    bb_monitor = BestBuyMonitor(redis_client, poll_interval=1800)
    # Seed the Samsung 980 Pro refurb if not already tracked
    bb_products = bb_monitor.list_products()
    if not any(p.get("product_id") == "JCQ6HQXJVH" for p in bb_products):
